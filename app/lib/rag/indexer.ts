...
async function deleteOldData(tenantSlug: string): Promise<void> {
  try {
    await getPinecone().index(INDEX_NAME).namespace(`${tenantSlug}-content`).deleteAll();
...
async function upsertChunks(chunks: Chunk[], tenantSlug: string): Promise<void> {
  const idx = getPinecone().index(INDEX_NAME).namespace(`${tenantSlug}-content`);
...
export async function runIndexing(
  tenantId: string,
  tenantSlug: string,
  seedUrls: { url: string; domain: string }[] = SEED_URLS,
): Promise<{ chunksIndexed: number; pagesScraped: number }> {
  console.log(` ${tenantSlug} indexing started`);
  console.log(` ${seedUrls.length} seed URLs`);
  ...
  const queue: { url: string; domain: string }[] = [...seedUrls];
  ...
  await deleteOldData(tenantSlug);
  await upsertChunks(allChunks, tenantSlug);
